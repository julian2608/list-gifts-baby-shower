...
import { Gift as GiftIcon, Trash2, ArrowLeft, Sparkles } from 'lucide-react';
...
export default function AdminPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: '', description: '', imageUrl: '', purchaseLink: '', isShared: false });
      setLoading(false);
      alert('¡Regalo agregado exitosamente! 🎁');
    } catch (error) {
      console.error('Error adding gift:', error);
      alert('Error al agregar el regalo. Por favor intenta de nuevo.');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          <div className="bg-white/80 backdrop-blur-sm rounded-3xl shadow-xl p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              >
                {loading ? 'Agregando...' : 'Agregar Regalo'}
              </button>
            </form>
          </div>
